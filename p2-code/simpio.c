...
void simpio_reset_terminal_mode() { tcsetattr(STDIN_FILENO, TCSANOW, &oldt); }
...
void iprintf(simpio_t *simpio, char *fmt, ...) {
    ...
    va_list myargs;
    va_start(myargs, fmt);
    off += vsnprintf(output + off, MAXLINE, fmt,
                     myargs); // add on the new message
    va_end(myargs);
    off += snprintf(output + off, max - off, "%s",
                    simpio->prompt); // add prompt back
    off += snprintf(output + off, max - off, "%s",
                    simpio->buf); // current typed input
