...
#define DISCONNECT_SECS                                                        \
    5 // seconds before clients are dropped due to lack of contact
...
typedef struct {
    ...
    char to_client_fname[MAXPATH]; // name of file (FIFO) to write into send to
                                   // client
    char to_server_fname[MAXPATH]; // name of file (FIFO) to read from receive
                                   // from client
    int data_ready; // flag indicating a mesg_t can be read from to_server_fd
    int last_contact_time; // ADVANCED: server time at which last contact was
                           // made with client
} client_t;

// server_t: data pertaining to server operations
typedef struct {
    char server_name[MAXPATH];   // name of server which dictates file names for
                                 // joining and logging
    ...
    sem_t *log_sem; // ADVANCED: posix semaphore to control who_t section of
                    // log file
} server_t;
...
typedef struct {
    ...
    char to_client_fname[MAXPATH]; // name of file server writes to to send to
                                   // client
    char to_server_fname[MAXPATH]; // name of file client writes to to send to
                                   // server
} join_t;
...
typedef enum {
    ...
    BL_SHUTDOWN =
        40, // server to client : server is shutting down, no name/body
...
typedef struct {
    ...
    int line_ready;       // flag determining if a line in buf is ready for
                          // processing
